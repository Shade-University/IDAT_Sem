...
public class ISKAMUserPageController {
    ...
    private void refreshData() throws SQLException {
        ...
        lVFoodOrders.setItems(FXCollections.observableArrayList(orderDAO.getOrdersByUser(loggedUser)));
        lVTodayOrders.setItems(FXCollections.observableArrayList(orderDAO.getTodayOrderByUser(loggedUser, Date.valueOf(LocalDate.now()))));
    }
    ...
    private void dateSearchChanged(ActionEvent event) throws SQLException {
        ...
        if (fm != null)
            lVFoodMenu.setItems(FXCollections.observableArrayList(productDAO.getProductByFoodMenu(fm)));
    }
    ...
    private void btnPayClicked(ActionEvent actionEvent) {
        try {
            if(txtCardNumber.getLength() > 8 && txtCVV.getLength()>2 && txtExpiration.getLength() > 4 && txtMoneyToDeposit.getLength() > 0){
                orderDAO.createOrder(new Order(-1, loggedUser, null, TRANSACTION_TYPE.IN, Float.valueOf(txtMoneyToDeposit.getText()), Date.valueOf(LocalDate.now()),"Dobití kreditu z karty " + txtCardNumber.getText()));
