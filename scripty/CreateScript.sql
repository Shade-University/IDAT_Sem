...
BEGIN
    ...
    INSERT INTO ZPRAVY(id_zprava, nazev, telo, datum_vytvoreni, id_uzivatel_odesilatel, id_uzivatel_prijemce,
                       id_skupina_prijemce)
...
BEGIN
    ...
    VALUES (skupina_id, nazev_in, popis_in, id_predmet_in);
END;
/
/*=====Insert procedury=====*/
/*=====Delete procedury=====*/
...
CREATE OR REPLACE TRIGGER zpravy_trigger
    ...
    FOR EACH ROW
BEGIN
    if(LENGTH(:NEW.nazev) <= 0 or LENGTH(:NEW.nazev) > 30) then raise_application_error(-20001, 'Název zprávy nesmí být prázdný nebo vìtší jak 30 znakù');
    elsif(LENGTH(:NEW.telo) <= 0 or LENGTH(:NEW.telo) > 250) then raise_application_error(-20001, 'Tìlo zprávy nesmí být prázdné nebo vìtší jak 150 znakù');
    end if;
